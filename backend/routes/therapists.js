...
router.get('/', async (req, res) => {
  try {
    const result = await db.query(`
      SELECT t.id, t.specialty, t.unit, t.color, t.bio,
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    // specialty é array
    const specialtyArr = Array.isArray(specialty) ? specialty : [specialty]
    await db.query(`UPDATE therapists SET specialty=$1, unit=$2, bio=$3 WHERE id=$4`,
      [specialtyArr, unit, bio, req.params.id])
    if (full_name || email) {
