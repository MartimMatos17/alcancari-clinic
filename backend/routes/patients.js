...
router.get('/', auth, async (req, res) => {
  try {
    ...
    if (search) {
      params.push('%' + search + '%')
      query += ` AND (p.full_name ILIKE $${params.length} OR u.full_name ILIKE $${params.length})`
    }
    if (gender) {
      params.push(gender)
      query += ` AND p.gender = $${params.length}`
    }
    query += ' GROUP BY p.id, u.full_name, u.email, u.phone ORDER BY p.full_name ASC'
...
router.get('/:id', auth, async (req, res) => {
  try {
    const result = await db.query(`
      ...
      WHERE p.id = $1
    `, [req.params.id])
    if (!result.rows[0]) return res.status(404).json({ error: 'Paciente nao encontrado' })

    const appts = await db.query(`
      SELECT a.*, t.full_name as therapist_name, s.name as service_name
      FROM appointments a
