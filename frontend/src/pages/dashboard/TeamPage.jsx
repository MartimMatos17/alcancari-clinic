...
const SPECIALTIES = ['Terapia Ocupacional', 'Psicologia', 'Terapia da Fala', 'Fisioterapia', 'Acupuntura', 'Outra']
const UNITS = [
...
export default function TeamPage() {
  ...
  const openEdit = (t) => {
    setSelected(t)
    setForm({ full_name: t.full_name || '', email: t.email || '', specialty: t.specialty || '', unit: t.unit || 'both', bio: t.bio || '' })
    setSaved(false)
    setModal('edit')
  }
  ...
  const save = async () => {
    ...
    try {
      if (modal === 'edit' && selected) {
        ...
        await api.post('/therapists', { ...form, password: 'password123' })
      }
      setSaved(true)
      load()
            ...
            return (
              <motion.div key={t.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full border font-medium mt-1 inline-block ${colorClass}`}>
                        {t.specialty}
                      </span>
                    </div>
                  </div>
                ...
                <div className="space-y-1.5 text-xs text-gray-400">
                  ...
                  {t.bio && <p className="text-gray-300 text-xs line-clamp-2 mt-2">{t.bio}</p>}
                </div>
              </motion.div>
            )
          })}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95 }}
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Especialidade *</label>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={save} disabled={saving || !form.full_name || !form.specialty}
        ...
        {deleteConfirm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <p className="text-gray-400 text-sm">Esta ação irá remover o terapeuta do sistema.</p>
